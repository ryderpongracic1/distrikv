...
- **`Apply` is idempotent in state, and gated in effects.** The state change is a map assignment, so a replay after a restart lands on the same view. Its *announcements* are not idempotent, and originally that was the whole story — which is [defect 14](defect-log.md#defect-14-health-log-replay-re-announced-historical-recoveries-on-every-restart). Because `lastApplied` is volatile and the view starts empty, a restart walks the post-snapshot log from "everything is healthy" again, so each historical `down`→`up` pair looked like a fresh failure and a fresh recovery: a `WARN` about a peer that is fine and a catch-up pass for a replica that never went anywhere, per pair, per restart. Raft is the only layer that can tell the cases apart — it alone knows which entries predate the process — so `RaftNode` records a **replay frontier** at open (the highest index already on disk) and routes entries at or below it to `StateMachine.ReplayApply` instead of `Apply`. The health state machine moves its state identically on either path and emits the `WARN`, the recovery `INFO` and the channel send only on the live one. **State idempotence is not effect idempotence**, and `RestoreFromSnapshot` had always drawn that line ("a snapshot is not an observation"); this extends it to the path that lacked it.
...
- **The transport probe is gone.** It was retained when this feature landed, as a fourth signal covering the leaderless window that a consensus-backed view cannot make progress through. That caution has since been discharged by measurement rather than argument: the [P2 gate](chaos-harness.md#the-p2-gate-passed) ran the whole chaos suite with the probe disabled, including the first-ever `leader-kill` runs, and convergence stayed inside the range the entire probe-ON history sits in. `cluster.PeerHealth` now has no probe function, no ticker and no goroutine, and health is carried by the committed log plus the write path's own outcomes — the twist as designed. One local override survives and is not probing machinery: a node whose own replication to a peer just succeeded may treat that peer as reachable despite a committed "down", which covers the asymmetric partition the gate could not inject. It works in the healthy direction only; see [replication-and-anti-entropy.md → The trigger](replication-and-anti-entropy.md#the-trigger).
...
- **`StateMachine`, and the KV coupling that is gone.** `internal/raft` previously took a `StoreInterface` and applied `put`/`delete` entries straight into the storage engine. That code was unreachable — nothing had ever proposed an entry, verified by grep across `cmd/`, `internal/server` and `internal/cluster`: nothing outside the raft package constructs a `kvpb.LogEntry` or sets `Entries` — and it described an architecture this system deliberately does not have. It has been replaced by a three-method `StateMachine` (`Apply`, `SnapshotState`, `RestoreFromSnapshot`) and Raft no longer imports the store at all.
    ...
    One consequence worth stating plainly, because it is a requirement on the health state machine rather than a detail: **`Apply` must be idempotent.** `commitIndex` and `lastApplied` are volatile, as in the paper, so after a restart entries between the snapshot boundary and the pre-crash frontier are applied again once the leader re-announces its commit index. `internal/raft/persistence_test.go` pins that replay so the requirement is observable rather than merely documented. Idempotent *state* is not enough on its own, though: an entry replayed that way must also emit no effect that only live evidence justifies, which is why Raft routes it to `ReplayApply` and the state machine announces nothing there ([defect 14](defect-log.md#defect-14-health-log-replay-re-announced-historical-recoveries-on-every-restart)). `internal/raft/replay_frontier_test.go` pins the boundary Raft draws, and `cmd/node/health_replay_gate_test.go` pins the state machine's side of it.
...
- **The transport probe is gone.** Covered above under "Consensus node health": it was kept as a fourth signal when this feature landed and removed once the P2 gate measured the configuration without it. What remains is the committed view, this node's own replication outcomes, and the leader's heartbeat outcomes — three signals, no dedicated probing.

**The discriminating test (`cmd/node/consensus_health_test.go`).** Everything above can be true while the design premise stays false, so one test is built to fail if it is. It stands up a 3-node in-process cluster, **starves every local signal** — the local tracker is never fed an observation and no probing machinery exists that could feed it one, and no replication RPCs are issued — and then observes a **follower**, a node that sends no heartbeats and therefore has no direct observation of anything. It isolates one of the other followers bidirectionally, and asserts the observing follower learns the peer is unhealthy, learns it is healthy again after the link is restored, and **schedules a replica catch-up pass** for it. With every local signal removed, the only path that information can have taken is the committed log. The same test asserts the leader's term does not move materially while the transitions flow, so the aggregator cannot be trading health for elections.
...
3. **Applying is at-least-once, not exactly-once, across a restart.** `commitIndex` and `lastApplied` are volatile, as in the paper, so a restarted node re-applies entries between the snapshot boundary and its pre-crash frontier. `StateMachine.Apply` must therefore be idempotent. Persisting the two indices would trade that requirement for an fsync on every commit advance, which is the wrong trade for a state machine whose entries are idempotent by nature ("node3 is down").
    ...
    What that requirement does **not** cover, and [defect 14](defect-log.md#defect-14-health-log-replay-re-announced-historical-recoveries-on-every-restart) is the price of assuming it did: a replayed entry must also emit no *effect* — no operator-facing warning, no notification on a channel — that only live evidence justifies. Raft supplies the classification rather than asking the state machine to guess, since only Raft knows which entries were on disk before the process started: entries at or below the **replay frontier** recorded at open go to `StateMachine.ReplayApply`, everything above it to `Apply`. The frontier is the persisted log tail rather than the pre-crash `lastApplied`, which is recorded nowhere, so an entry persisted but never applied before a crash is conservatively treated as a replay — silence in a case where the alternative is noise, at the cost of a delayed catch-up pass the anti-entropy retry loop already covers.
